<template>
  ...
  >
    <template #header>
      <Link v-if="hasPermission('CREATE_STATUSES')" :href="route('statuses.create')">
        <PrimaryButton> Add New Status </PrimaryButton>
      </Link>

      <Link v-if="hasPermission('CREATE_STATUSES')" :href="route('role_statuses.index')">
        <span
...
const deleteStatus = (statusId, name) => {
  const message = "Are you sure you want to delete the selected status `" + name + "` ?";
