...
const updateRolePemission = (permissionId, event) => {
  ...
  const permissionSuccessMsg = event.target.checked ? 'given to' : 'removed from';


const message =
  'Are you sure you want to ' + permissionMsg + ' permission to the selected role ' +
  props.role.name + ' ?';


  confirmDialogBox(message, () => {
    router.post(route('roles.assign.permission.to.role', params), {}, {
      onSuccess: () => {
        showSuccessNotification("Permission "+permissionSuccessMsg+" the role");
...
const attachGroupPermisisonToRole = (value) => {
  const message =
    'Adding group permission `' + value?.name + '` to ' + props.role.name + ' role?';

  confirmDialogBox(message, () => {
    axios.post(route('roles.attach.group.permissions'), {
...
const detachGroupPermisisonToRole = (value) => {
  const message =
    'Are you sure you want to remove group permission `' + value?.name + '` from ' + props.role?.name + ' role?';

  confirmDialogBox(message, () => {
    axios.post(route('roles.detach.group.permissions'), {
      permission_group_id: value.id,
      role_id: props.role.id,
    })
    .then((response) => {
