<template>
  <div class="w-full max-w-full px-2 sm:px-4 rounded-2xl min-h-[500px] sm:min-h-[742px] flex flex-col">
    ...
    <!-- Table Container -->
    <div class="overflow-x-auto mt-5 rounded-xl shadow-md relative">
      <!-- Loading Overlay -->
      <div v-if="state.isDataFetching" class="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex items-center justify-center">
      ...
      <table class="w-full min-w-[600px] table-auto border border-gray-200 rounded-lg">
        <thead class="bg-gray-50/90 backdrop-blur-sm sticky top-0 z-5">
          <tr>
            <th
              v-for="(column, index) in filterColumns"
              :key="'header-' + column.key"
              :class="getHeaderClass(column, index) + ' px-6 py-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider group'"
            >
              <div
                :class="column.sortable ? 'flex items-center justify-between cursor-pointer hover:text-primary-600 transition-colors duration-200' : ''"
              ...
              >
                ...
                <div v-if="column.sortable" class="ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <ChevronUp
        ...
        <tbody class="divide-y divide-gray-200/60 bg-white">
          <template v-if="state.isDataFetching">
            ...
            <tr v-for="n in state.perPage" :key="'loading-table-content-' + n" class="animate-pulse">
              <td v-for="(column, colIndex) in filterColumns" :key="'skeleton-' + colIndex" class="px-6 py-5">
                <div :class="colIndex === 0 ? 'flex items-center space-x-4' : ''">
                  <div v-if="colIndex === 0" class="h-10 w-10 bg-gray-200/60 rounded-full"></div>
                  <div :class="colIndex === 0 ? 'flex-1 space-y-3' : ''">
                    <div class="h-4 bg-gray-200/60 rounded-full" :class="colIndex === 0 ? 'w-3/4' : (colIndex % 2 === 0 ? 'w-2/3' : 'w-1/2')"></div>
                    <div v-if="colIndex === 0" class="h-3 bg-gray-200/60 rounded-full w-1/2"></div>
                  </div>
                </div>
              </td>
            </tr>
          ...
          <template v-else>
            <tr
              ...
              :class="getTrClass(record) + ' transition-all duration-300 hover:bg-gray-50/80 hover:shadow-sm'"
              class="fade-in-row"
            >
              <td
                v-for="(column, columnIndex) in filterColumns"
                :key="'body-' + column.key"
                :class="getBodyClass(column, columnIndex) + ' px-6 py-5 text-sm text-gray-700 break-words'"
              >
                <slot
                  v-if="!column.hidden"
                  :name="`${column.key}`"
      ...
      <!-- Empty State -->
      <div v-if="!state.isDataFetching && state.records.length === 0" class="text-center py-16 px-6">
